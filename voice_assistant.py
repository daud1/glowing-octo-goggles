...
if not ELEVEN_LABS_API_KEY:
    raise ValueError("Missing API_KEY for ElevenLabs!")
    ...
    def generate_voice_response(self, text: str, voice_name: str = None) -> str:
        ...
        try:
            ...
            audio_generator = self.client.text_to_speech.convert(
                text=text, voice_id=selected_voice, model_id="eleven_multilingual_v2"
            )
...
class VoiceAssistantRAG:
    def __init__(self):
        ...
        self.voice_generator = VoiceGenerator(ELEVEN_LABS_API_KEY)
    ...
    if uploaded_files and st.button("Process Documents"):
        with st.spinner("Processing documents..."):
            ...
            try:
                ...
                # Create vector store
                vector_store = doc_processor.create_vector_store(processed_docs, "knowledge_base")
...
def main():
    ...
    if not all([elevenlabs_api_key, openai_api_key]):
        st.error("Please set ELEVEN_LABS_API_KEY and OPENAI_API_KEY in your environment variables")
        return
        ...
        try:
            ...
            if available_voices:
                selected_voice = st.sidebar.selectbox(
                    "Select Voice",
                    available_voices,
                    index=(available_voices.index("Rachel") if "Rachel" in available_voices else 0),
                )
            else:
